<?php
    ...
    Plugin URI: https://www.powr.io/plugins/photo-editor
    ...
    */
    ...
    class Powr_Photo_Editor extends WP_Widget{
      ...
      public function form( $instance ){
        ...
        ?>
        <p>
          To edit, visit your live webpage and click the gears icon on your Photo Editor.
        </p>
        <p>
    ...
    function powr_photo_editor_shortcode( $atts ){
      if(isset($atts['id'])){
        $id = $atts['id'];
      	return "<div class='powr-photo-editor' id='$id'></div>";
      }else if(isset($atts['label'])){
        $label = $atts['label'];
		    return "<div class='powr-photo-editor' label='$label'></div>";
      }else{
      	"<div class='powr-photo-editor'></div>";
      }
    }
    ...
    //ADD MENUS
    add_action( 'admin_menu', 'powr_photo_editor_menu' );
    function powr_photo_editor_menu() {
      add_menu_page( 'POWr Photo Editor', 'POWr Photo Editor', 'manage_options', 'powr-photo-editor-settings', 'powr_photo_editor_options', plugins_url('/powr-icon.png',__FILE__));
    }
    function powr_photo_editor_options() {
      if(powr_local_mode()){//Determine JS url:
        $redirect_url = 'https://localhost:3000/wp-create/photo-editor';
      }else{
        $redirect_url = 'https://www.powr.io/wp-create/photo-editor';
      }
    ...
    register_activation_hook(__FILE__, 'powr_photo_editor_plugin_activate');
      add_action('admin_init', 'powr_photo_editor_plugin_redirect');

      function powr_photo_editor_plugin_activate() {
      add_option('powr_photo_editor_plugin_do_activation_redirect', true);
      }
      ...
      function powr_photo_editor_plugin_redirect() {
      if (get_option('powr_photo_editor_plugin_do_activation_redirect', false)) {
          delete_option('powr_photo_editor_plugin_do_activation_redirect');
          if(!isset($_GET['activate-multi']))
          {
            wp_redirect( get_admin_url().'?platform=wordpress&page=powr-photo-editor-settings&' );
          }
       }
      }
